...
class Dispatcher_Workspace(object):
    ...
    def Core_running(self, core_obj, run_type="WCET"):
        while True:
            ...
            # 3.执行job，y的WCET或者WCET与BCET中的随机数
            if run_type == "WCET":
                yield self.env.timeout(job_node[1]['WCET'])
            elif run_type == "AVE":
                yield self.env.timeout(job_node[1]['AVE'])
            elif run_type == "BCET":
                yield self.env.timeout(job_node[1]['BCET'])
            elif run_type == "RANDOM":
                yield self.env.timeout( randint(job_node[1]['BCET'], job_node[1]['WCET']) )
...
if __name__ == "__main__":
    envi = simpy.Environment()  # create a environment for simulation
