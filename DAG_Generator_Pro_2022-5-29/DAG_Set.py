...
class DAG_Set:
    ...
    def Random_DAG_Set(self, DAG_count, parallelism_list, critical_path_list):
        for x in range(0, DAG_count):
            ...
            # step1. DAG结构生成
            Temp_DAG.DAG_Generator("mine")
            # step2. DAG WCET配置；
            Temp_DAG.WCET_Config("random", 10, 100)
            # step3. DAG 优先级配置；
            Temp_DAG.Priority_Config("random")
            # step4. DAG 关键路径配置；
            Temp_DAG.critical_path_config()
            # stpe5. 加入DAG集合
            self.Dag_Set.append(Temp_DAG)
    ...
    def Status_Dataup(self):
        # 将Temp_DAG_Set中前驱节点为0且未就绪的节点的节点进入就绪队列
    ...
    def get_dag(self, DAG_ID):
        ret = [x for x in self.Dag_Set if x.DAG_ID == DAG_ID]
        if len(ret) == 1:
            return ret[0]
        elif len(ret) == 0:
            print("\n\nDAG:{0} not exist!\n\n".format(DAG_ID))
    ...
    def get_priority_ready_node_list(self):
        temp_dict = {}
        ret_list = []
        for x in self.Dag_Set:
            temp_dict[x] = x.Priority
...
"""
        ...
        # self.Dag_Set.append(G1)
        self.Dag_Set.append(G2)
        # self.Dag_Set.append(G3)
        # self.Dag_Set.append(G4)
